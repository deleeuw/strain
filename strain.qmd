...
\end{equation}
for some given function $f$. @groenen_deleeuw_mathar_C_95 call \eqref{eq-fstress} fStress. For $f$ the identity we have stress (@kruskal_64a), for $f$ the square we have sstress (@takane_young_deleeuw_A_77), and for $f$ the logarithm we have log-stress (@ramsay_77). 

If we ravel the matrices $\Delta$ and $D(X)$ to vectors we can write
\begin{equation}
...
$$
(\delta - d)'J\otimes J(\delta - d)
$$
$$
\delta=\tilde\delta+(\delta-\tilde\delta)
$$
$$
((\delta-\tilde\delta) - (d-\tilde\delta))'J\otimes J((\delta-\tilde\delta) - (d-\tilde\delta))\leq
\sigma(\tilde\delta)-2(d-\tilde\delta)'J\otimes J(\delta-\tilde\delta)+\|\delta-\tilde\delta\|^2 
$$
$$
