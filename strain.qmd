...
In non-metric MDS we have to minimize strain over both $X$ and $\Delta$, where $\Delta$ varies in some subset of the space $\mathcal{D}$ of non-negative, symmetric and hollow matrices of order $n$. 

In @trosset_97, @trosset_00, @trosset_98, @trosset_02 there is a nice overview of previous attempts to minimize non-metric strain in MDS. 
...
Normalization is based on strain as expressed in Equation \eqref{eq-strainc}.
We minimize strain over $X$ for fixed $\Delta$ under the constraint $\text{tr}\ (X'X)^2=1$. This is equivalent to maximizing $\text{tr}\ X'CX$ over 
$\text{tr}\ (X'X)^2=1$.
The stationary equations are
$$
CX=\gamma X(X'X)
$$
If the SVD of $X$ is $K\Lambda L'$, then we must have $\text{tr}\ \Lambda^4=1$
and
$$
CK\Lambda=\phi K\Lambda^3
$$
from which $K\Lambda$ are eigenvectors of $C$ with eigenvalues $\phi\Lambda^2$.
