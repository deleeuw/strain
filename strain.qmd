...
We will also use another representation of strain in this paper. By applying classical embedding result of @schoenberg_35 and @young_householder_38, in the version proposed by @torgerson_58, we find
\begin{equation}
...
In *metric* MDS the matrix $\Delta$ is constant and given, and we minimize
strain over $X$. From \eqref{eq-strainc} the basic theorem of classical scaling (@torgerson_58), also known as  principal coordinate analysis (@gower_66), tells us that the optimal $X$ are the eigenvectors corresponding with the $p$ largest eigenvalues of $C_+$, the positive definite part of $C$, with each eigenvector scaled to have norm equal to the square root of the corresponding eigenvalue There are rigorous proofs in @keller_62 and @deleeuw_R_74c.

In non-metric MDS we have to minimize strain over both $X$ and $\Delta$, where $\Delta$ varies in some subset of the space $\mathcal{D}$ of non-negative, symmetric and hollow matrices of order $n$. Classical examples, treated in detail
below in this paper, are the additive constant problem, the missing data problem, the use of bound constraints, and ordinal non-metric scaling.

The history of various tentative and/or incomplete attempts to construct a non-metric MDS technique based on strain is in @deleeuw_heiser_C_82 and @trosset_98. The first systematic approach to the problem is @deleeuw_R_74b,
who discusses the additive constant, the missing data, and the ordinal forms
of MDS. There is not software, only equations. The paper uses what 
@trosset_97 calls "variable reduction". The minimum of strain over $X$,
which is the sum of squares of the discarded eigenvalues of $C_+$, is minimized over $\Delta$. Results from classical perturbation theory are used to 
give expression for the first and second derivatives of the "reduced"
loss function, with the implicit understanding that these derivatives
can be used in a large variety of general-purpose optimization methods.
An implementation of the same methodology for the additive constant problem
is in @saito_78. Since the "reduced" strain in this cases only depends on a single parameter (the additive constant) Saito uses a simple bisection-type
search procedure on the positive real line. 

A more systematic and general approach was followed by Michael Trosset in a sequence of papers (@trosset_97, @trosset_00, @trosset_98, @trosset_02).

The next non-metric scaling problem occurs when
...
non-negative. An important speciasl case of the missing data
problem is the unfolding problem, in which we have similarities between objects in two disjoint sets. The between-set distances are known, while both 
...
The second subproblem, minimizing strain of $\Delta$ for fixed $X$ (and thus fixed $D$), can take various forms. We discuss some of the important special cases, and their implementation in the following sections.
...
From \eqref{eq-addone} we have, with 
\begin{subequations}
\begin{align}
...
is required to be in a given interval (which may be infinitely large). Updating $\Delta$ for given $X$ is a quadratic programming problem. By choosing the interval to be the non-negative real axis this gives us another way, using majorization, to impute missing data. For non-missing data the interval
has length zero and is equal to the dissimilarity $\delta_{ij}^0$.

Strain for this problem 
...
\begin{equation}
\sigma(\Delta)=\frac14\text{tr}\ J(\Delta^2 - D^2)J(\Delta - D^2)J\label{eq-straindel}
\end{equation}
...
Normalization: $G=J\Delta^2 J$ $\text{tr}\ G^2=1$.
...
For the four special cases we have discussed  there are four R programs (@r_core_team_25) in the github repository.
